    ...
    if(localStorage.getItem("superheroes"))
        superheroes = JSON.parse(localStorage.getItem("superheroes"));
            ...
            for (let superhero of superheroes) {
                ...
                <div class="hero-pic"><img src="${superhero.pic}"></div>
                <div class="hero-info"><h2>${superhero.name}</h2>
                <div>Вселенная: ${superhero.universe}</div>
                <div>Альтер-эго: ${superhero.alterEgo}</div>
                <div>Род деятельности: ${superhero.occupation}</div>
                <div>Друзья: ${superhero.friends}</div>
                <div>Суперсилы: ${superhero.powers}</div><hr></div>
                <div class="hero-details"><div>${superhero.details}</div><hr>
                <span>Оценка: <input id="rate${i}" type="number" max="5"></span>
                <button onclick="rateHero(${i});">Оценить</button>
                <div id="result${i}">${(superhero.rate ? superhero.rate :"")}</div>
                </div>
                </div><br>`;
                i++;
            }
...
function rateHero(id){
    let rate = document.getElementById('rate'+id).value;
document.getElementById('result' + id).innerHTML = `Вы поставили оценку:${rate}`;
superheroes[id].rate = rate;
